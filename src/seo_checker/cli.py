...
def print_results_as_tables(results: Dict[str, Any], url: str):
    ...
    # Layout output removed

    # (Spelling table printed earlier after Locations)
    ...
    for src in (im.get('missing_alt_no_title') or [])[:20]:
        issues_rows.append(["Missing alt (no title)", _truncate(src, 120)])
    for src in (im.get('missing_alt_with_title') or [])[:20]:
        issues_rows.append(["Missing alt (has title)", _truncate(src, 120)])
    for val in (im.get('poor_alt') or [])[:20]:
        issues_rows.append(["Weak alt", _truncate(val, 120)])
    if issues_rows:
        _print_table(
            "Image Alt Issues",
            ["Issue", "Src/Alt"],
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    for site_url in unique_urls:
        ...
        if not args.quiet and args.format in ('table', 'both') and not is_child:
            print_results_as_tables(result, site_url)

    if not args.quiet and len(outputs) == 2 and args.format in ('table', 'both'):
